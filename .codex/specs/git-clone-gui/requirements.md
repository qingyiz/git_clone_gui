...
| FACT-036 | 用户反馈当前整体界面“一眼就是 AI 做的”、过度 AI 化，要求优化。基线截图中深色大侧栏、高饱和蓝色整块选中、多层白色圆角卡片、胶囊标签和较大留白同时出现。 | 用户明确且已验证 | 2026-08-17 用户反馈；`before-clone.png` / `before-workspace.png`；`AppStyle.cpp` 与页面 UI 组装 | REQ-006 需从通用“卡片化现代界面”收敛为低饱和、高信息密度、少容器嵌套的成熟桌面工具视觉，两个页面和导航壳必须统一。 |
| FACT-037 | 用户指出优化后侧栏左上角仍保留蓝色方块“G”图标，询问为什么不替换。截图确认该图标为 26px 高饱和蓝色圆角块内的单字母，没有 Git/仓库语义。 | 用户明确且已验证 | 2026-08-17 用户截图；`MainWindow::createUi()` 的 `QLabel("G")`；`AppStyle.cpp` 的 `navigationMark` 样式 | 侧栏品牌标识应替换为有 Git 分支语义、低饱和且跨平台一致的自绘图标，不继续使用字母占位。 |
| FACT-038 | 用户确认本轮界面与品牌图标优化可以按既有流程发布到 GitHub，并要求创建 tag 与 Release；当前最新不可移动标签/Release 为 `v0.1.5`。 | 用户明确且已验证 | 2026-08-17 用户指令；`git tag`、GitHub Release/仓库状态 | 使用下一个补丁版本 `v0.1.6`，先经 PR 合并 `main`，再从合并提交创建标签并由既有双平台 workflow 发布。 |
...
- 使用成熟桌面工具的紧凑层级：低饱和中性底色、少量强调色、较少容器嵌套、小圆角和完整悬停/聚焦/禁用状态。
...
### REQ-006：提供紧凑、一致且有桌面工具质感的界面

**用户故事：** 作为开发者，我希望界面像经过长期打磨的桌面工具，从而快速扫读和操作，而不被模板化的大卡片、高饱和色块或过量留白干扰。
...
- AC-006.2：父项目、子仓库、目标位置和执行信息应使用统一 4/8/12/16 像素紧凑间距层级；容器以 6～8px 小圆角、1px 中性分隔线为主，不应通过大面积高对比色块和多层卡片嵌套建立层级。
...
- AC-006.6：左侧导航应收紧为低对比中性侧栏，当前页使用小面积指示和文字强调，不使用占满整行的高饱和蓝色块；版本信息应降级为次要文本而非独立胶囊卡。
- AC-006.7：两个页面的大标题、说明文案、状态标签和操作按钮应保持较小字阶与信息密度；蓝色仅用于主操作、聚焦、选中指示和关键链接，成功/警告/错误仅在对应语义状态中使用。
- AC-006.8：侧栏品牌标识应使用语义明确的 Git 分支节点线性图标，不显示单字母“G”、emoji 或依赖操作系统主题图标；图标应使用低饱和浅色底、中性边框和蓝灰分支线，在 Qt 5/6 与 Retina/高 DPI 下形状保持清晰。
...
- AC-011.11：本轮界面优化应将单一项目版本提升为 `0.1.6`，通过 PR 合并到 `main` 后在该合并提交创建不可移动的 `v0.1.6` 注释标签；`docs/releases/v0.1.6.md` 应说明低饱和紧凑视觉、Git 分支品牌图标、功能保持、平台附件与签名提示。标签流水线的 macOS、Windows 和 Release job 全部成功，且公开 Release 正文、两个附件和校验值均可核验后才视为完成。
...
| NFR-018 | 视觉节制 | 默认窗口中导航选中区不使用整行高饱和色块；普通容器圆角不超过 8px；胶囊标签仅保留状态/计数语义；主操作蓝色面积应明显小于中性底色和内容区 | QSS/对象属性审查、克隆页与工作区页默认截图对比、macOS 人工视觉检查 |
...
| ANA-031 | 视觉根因 | REQ-006 | 旧 DEC-007 要求浅色背景、白卡、12px 圆角和蓝色主色，与后续深色侧栏、胶囊徽标组合后形成高度模板化的“AI 仪表盘”观感 | 保留功能区和双页结构，重定义视觉 token 与容器层级；不借机重构页面业务、service 或配置契约 |
| ANA-032 | 品牌标识漂移 | REQ-006 | TASK-039 收紧了侧栏与色块，但仍保留 `QLabel("G")` 占位图标，与“去 AI 化”的语义图形目标不一致 | 新增 presentation 内无 I/O 的小型自绘组件，绘制 Git 分支节点；不复用平台 theme icon、emoji 或 app `.icns`，避免平台差异和小尺寸缩放失真 |
| ANA-033 | 发布版本选择 | REQ-011 | 当前 `v0.1.5` 已公开发布，移动或覆盖会破坏既有下载与源码可追溯性；本轮只需发布已验收的表示层改动 | 使用 `v0.1.6` 补丁版本，沿用 DEC-027 的单一版本源与标签流水线；先合并 PR，再从 `main` 合并提交打标签，不手工上传未经双平台验证的包 |
...
| REQ-006 | AC-006.1～AC-006.8 | 对象/尺寸测试、品牌标识语义/像素断言、克隆页/工作区截图对比、QSS token 与对象属性检查 |
...
| REQ-011 | AC-011.1～AC-011.11 | workflow/schema 审查、版本一致性、macOS/Windows 原生 Actions run、artifact 结构、Release 说明、ad-hoc Bundle 严格签名验证、Developer ID/公证门控、tag Release 与附件校验值 |
